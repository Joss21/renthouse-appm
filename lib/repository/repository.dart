...
class Repository {
  ...
  Future<int> authenticateUser(String email, String password) async {
    final List<DocumentSnapshot> docs =
        await _firestoreProvider.authenticateUser(email, password);

    if (docs.length == 0) {
      print("len 0");
      return 0;
    } else {
      String firstName = docs[0].data.containsKey("firstName")
          ? docs[0].data["firstName"]
          : "N/A";
      String lastName = docs[0].data.containsKey("lastName")
          ? docs[0].data["lastName"]
          : "N/A";
      String image =
          docs[0].data.containsKey("image") ? docs[0].data["image"] : "N/A";
      String password = docs[0].data.containsKey("password")
          ? docs[0].data["password"]
          : "N/A";
      String email =
          docs[0].data.containsKey("email") ? docs[0].data["email"] : "N/A";
  ...
  Future<UserModel> getUserByEmail(String email) async {
    final List<DocumentSnapshot> docs =
        await _firestoreProvider.getUserByEmail(email);
    ...
    if (docs.length == 0) {
      ...
      String email =
          docs[0].data.containsKey("email") ? docs[0].data["email"] : "N/A";
      String phone =
          docs[0].data.containsKey("phone") ? docs[0].data["phone"] : "N/A";
